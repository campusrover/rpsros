...
class Roamer:
    ...
    def facing_away_from_wall(self):
        return abs(self.shortest_bearing) > 170
    ...
    def parallel_wall(self):
        return GOAL_BEARING - 15 < self.shortest_bearing < GOAL_BEARING + 15
    ...
    def smart_lidar_cb(self, msg):
        ...
        if self.danger_zone() and self.facing_away_from_wall():
            ...
            self.twist = self.make_twist(0.3, 0.0)
        if self.danger_zone() and not self.facing_away_from_wall():
            self.print_debug("turnsaf,")
            self.twist = self.make_twist(0.0, 0.3)
        elif self.working_zone() and not self.parallel_wall():
            self.print_debug("turnsaf2,")
            self.twist = self.make_twist(0.0, 0.4)
            ...
            self.twist = self.make_twist(0.0, 0.5)
        else:
